...
pub fn validate(root: &Path) -> Result<(), String> {
    ...
    if manifest.schema != "anasemble-brand-asset-manifest-v1" || manifest.brand_version != "1.2.1" {
...
fn validate_tokens(path: &Path) -> Result<(), String> {
    ...
    if value["version"] != "1.2.1" {
        return Err("brand token version must match brand 1.2.1".into());
    }
    for pair in [
        ("#0B1628", "#FFFFFF"),
        ("#334155", "#FFFFFF"),
