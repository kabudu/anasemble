...
## P5 - supported cloud profiles (complete)
...
- [x] Retain clean-source native Amazon Linux 2023 evidence for arm64 and x86_64 on exact EC2 instance and AMI profiles.
- [x] Execute destructive restore and rollback against Amazon RDS for PostgreSQL, ElastiCache for Redis with in-transit encryption, and Amazon S3 over HTTPS.
- [x] Prove deny-egress NetworkPolicy enforcement and the integrated activation lifecycle on one exact Amazon EKS and VPC CNI profile.
- [x] Delete every tagged AWS fixture and retain zero-live-resource service scans plus bounded cost evidence.
- [x] Exit: the compatibility contract promotes only the exact AWS profiles exercised by retained evidence, and local CI remains green.
