...
`v0.1.0-rc.1` is the first public prerelease. `v0.1.0-rc.2` publishes the generic evidence protocol crates (`anasemble-core`, `anasemble-evidence`, `anasemble-events`) alongside the existing `anasemble` CLI package. `v0.1.0-rc.3` adds `anasemble-trace`, `anasemble-semantic`, `anasemble-contract`, and `anasemble-verification`. Hosted recovery, real service
