...
| BRD-2 | Establish an enduring owner-selected identity independently of maturity | Semantic Fit platform, three-fragment open-interface mark, complete vector family and separate maturity overlays | concept decision record and optical size/monochrome review in `BRAND_VALIDATION.md` |
| BRD-3 | Make brand assets accessible, safe, licensed and reproducible | machine-readable tokens, redundant semantic shapes, sanitised SVG, owned-assets notice, deterministic exports and digest manifest | `brand_assets check`, brand unit tests and repository checker |
| BRD-4 | Govern brand claims and cross-channel consistency | permanent/maturity messaging layers, qualified/prohibited vocabulary, role ownership, versioning and archive rules | brand guide audit plus maturity-term scan of canonical assets |
| OSS-1 | Distribute project-authored source and assets under Apache License 2.0 without implying trademark rights | root licence and notice, Cargo SPDX metadata, brand licence update and separate trademark policy | repository metadata check and GitHub community profile after opening |
| OSS-2 | Admit public contributions without weakening security, claims or support boundaries | contribution, conduct, governance, support, security and structured issue/PR policies | repository checker plus public-opening community-profile verification |
| OSS-3 | Keep dependency redistribution legally reviewable | explicit permissive Cargo Deny licence allowlist and locked dependency graph | `cargo deny --locked check licenses` in local CI |
| REL-6 | Prepare a curated release without conflating source opening, package publication, artifact publication or deployment | versioned title/body, exact artifact contract and fail-closed opening runbook | repository checker, Cargo package inventory, history secret scan and ordered publication gates |
| REL-7 | Publish installable artifacts with exact source and target identity | annotated tag, one crates.io package, native AL2023 arm64/x86_64 archives, checksums and provenance | package verification, native build smoke tests and release-asset digest audit |
| WEB-1 | Present the product publicly without adding a hosted application or data trust boundary | static TailwindCSS site, no browser JavaScript, telemetry, forms, remote resources or runtime dependencies | repository site-policy checker, deterministic CSS build, desktop/mobile render and link audit |
